        ...
        <section className="w-full max-w-[975px] mx-auto pb-16 mt-[80px]">
            <div className="flex justify-center">
                ...
                >
                    ...
                    <p className="text-[14px] tracking-[0.7px] ml-[18px]">CREATE NEW FOOD POST</p>
                </Button1>
            </div>
                        ...
                        <div className="flex flex-col lg:flex-row gap-5">
                            ...
                            <div className="flex-1">
                                <div className="flex items-start justify-between gap-4">
                                    <h3 className="text-[18px] font-bold text-black pl-1">{post.title}</h3>
                                    <Button1
                                        variant="outline"
                                        color="green"
                                        size="sm"
                                        className="w-[47px] h-[30px] rounded-[10px]"
                                    >
                                ...
                                <div className="mt-4 p-1 grid md:grid-cols-2 gap-x-8 gap-y-2 text-[13px] text-[var(--text-grey-4)]">
                                    <p><span className="font-bold text-[var(--text-grey-5)] mr-[30px]">Category</span>{post.category}</p>
                                    <p className="md:row-span-4 flex items-start"><span className="font-bold text-[var(--text-grey-5)] mr-[30px]">Description</span>{post.description}</p>
                                    <p><span className="font-bold text-[var(--text-grey-5)] mr-[55px]">Price</span>{post.price}</p>
                                    <p><span className="font-bold text-[var(--text-grey-5)] mr-[35px]">In Stock</span>{post.inStock}</p>
                                    <p><span className="font-bold text-[var(--text-grey-5)] mr-[12px]">Expiry Date</span>{post.expiryDate}</p>
                                    <p className="md:row-span-2 flex items-start"><span className="font-bold text-[#40403e] mr-[16px]">Ingredients</span>{post.ingredients}</p>
                                    <p><span className="font-bold text-[var(--text-grey-5)] mr-[50px]">Contact</span>{post.contact}</p>
                                    <p><span className="font-bold text-[var(--text-grey-5)] mr-[11px]">Email Address</span>{post.email}</p>
                                    <p className="flex items-start gap-2">
                                        <span className="font-bold text-text-[var(--text-grey-5)] mr-[26px]">Address</span>
                                        {post.address}
                                    </p>
                                </div>
                                ...
                                <Button1
                                    ...
                                    color="orange"
                                    size="sm"
                                    className="mt-4 rounded-[10px] text-[12px] px-[18px] py-[10px]"
                                >
                                    DELETE POST
                                </Button1>
                            </div>
